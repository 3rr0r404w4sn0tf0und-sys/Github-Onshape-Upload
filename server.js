...
app.get("/auth/onshape", (req, res) => {
  const url = `https://cad.onshape.com/oauth/authorize?response_type=code&client_id=${OS_CLIENT_ID}&redirect_uri=${encodeURIComponent(APP_URL + "/auth/onshape/callback")}`;
  res.redirect(url);
});
...
app.get("/api/parts", requireAuth, async (req, res) => {
  ...
  try {
    ...
    // which may originate from several different Part Studios.
    ...
    const parts = (data.rootAssembly?.instances || [])
      ...
      .map((inst) => ({
        ...
        sourceWorkspaceId: inst.documentMicroversion ? undefined : inst.elementId, // best-effort - see note below
...
async function exportPartsAsStep(onshapeToken, documentId, workspaceId, sourceElementId, partIds, merged, formatName) {
  const translateUrl = `https://cad.onshape.com/api/v6/partstudios/d/${documentId}/w/${workspaceId}/e/${sourceElementId}/translations`;
...
app.post("/api/commit", requireAuth, async (req, res) => {
  ...
  const { documentId, workspaceId, deletes = [], items = [] } = req.body;
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (item.isStatic) {
        const distinctSources = new Set(item.parts.map((p) => p.sourceElementId));
      ...
      let buffer;
      if (item.isStatic) {
        const sourceElementId = item.parts[0].sourceElementId;
        buffer = await exportPartsAsStep(req.session.onshapeAccessToken, documentId, workspaceId, sourceElementId, item.parts.map((p) => p.partId), true, formatName);
      } else {
        const { sourceElementId, partId } = item.parts[0];
        buffer = await exportPartsAsStep(req.session.onshapeAccessToken, documentId, workspaceId, sourceElementId, [partId], false, formatName);
      }
