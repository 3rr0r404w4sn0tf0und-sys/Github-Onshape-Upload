...
async function exportPartsAsStep(onshapeToken, documentId, workspaceId, elementId, partIds, merged) {
...
app.post("/api/commit", requireAuth, async (req, res) => {
  ...
  try {
    ...
    for (const item of items) {
      const buffer = await exportPartsAsStep(req.session.onshapeAccessToken, documentId, workspaceId, elementId, item.partIds, item.isStatic);
